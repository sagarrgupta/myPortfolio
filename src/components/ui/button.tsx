...
export interface ButtonProps
  ...
  asChild?: boolean;
  /** When false, Button does not add cursor-can-hover (use on parent Link/wrapper for unified hover) */
  cursorCanHover?: boolean;
  children?: ReactNode;
}
...
const Button = forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant, size, asChild = false, cursorCanHover = true, children, ...props }, ref) => {
    const Comp = asChild ? Slot : "button";
    return (
      <Comp
        className={cn(
          buttonVariants({ variant, size, className }),
          cursorCanHover && "cursor-can-hover"
        )}
        ref={ref}
        {...props}
      >
